...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('rag_generation.log'),
...
class ResponseGenerator:
    """Generate evaluation responses using different RAG configurations."""
    ...
    def create_rag_system(self, config: Dict) -> RAGSystem:
        ...
        if config["vector_db"] == "chroma":
            vector_db = ChromaDB(collection_name=f"eval_{config['name']}")
        else:
            vector_db = RedisDB(collection_name=f"eval_{config['name']}")
    ...
    def generate_responses(self) -> None:
        ...
        for config in self.configurations:
            ...
            try:
                ...
                if not os.path.exists(self.data_dir / "processed"):
                    logger.info("Processing documents...")
                    rag.ingest_documents(str(self.data_dir))
                    (self.data_dir / "processed").touch()
    ...
    def generate_summary(self) -> None:
        ...
        logger.info(f"Summary saved to {self.results_dir}/summary.json")

def main():
    parser = argparse.ArgumentParser(description='Generate evaluation responses for RAG system')
