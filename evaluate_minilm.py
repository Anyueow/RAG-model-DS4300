"""Evaluation script for RAG pipeline with all-MiniLM-L6-v2 model."""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('minilm_evaluation.log'),
...
VECTOR_DBS = {
    "chroma": lambda: ChromaDB(collection_name="minilm_eval_collection"),
    "redis": lambda: RedisDB(collection_name="minilm_eval_collection"),
    "qdrant": lambda: QdrantDB(collection_name="minilm_eval_collection")
}

def evaluate_config_task(vector_db: str, llm_model: str,
    ...
    try:
        ...
        model_config = EMBEDDING_MODELS["multi-qa-MiniLM-L6-cos-v1"]
...
class MiniLMPipelineEvaluator:
    """Evaluator for RAG pipeline with all-MiniLM-L6-v2 model."""
    ...
    def save_results(self):
        ...
        if not self.results:
            logging.error("No results to save. Evaluation may have failed.")
            return
            
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        # Save to CSV
        csv_file = f"minilm_evaluation_{timestamp}.csv"
        df = pd.DataFrame(self.results)
        df.to_csv(csv_file, index=False)
        
        # Save to JSON
        json_file = f"minilm_evaluation_{timestamp}.json"
    ...
    def generate_summary(self, df: pd.DataFrame, timestamp: str):
        ...
        """
        if df.empty:
            logging.error("No data to generate summary from.")
            return
            
        summary_file = f"minilm_evaluation_summary_{timestamp}.txt"
        
        with open(summary_file, 'w') as f:
            f.write("=== RAG Pipeline Evaluation Summary (all-MiniLM-L6-v2) ===\n\n")
            
            # Overall statistics
            f.write("Overall Statistics:\n")
            try:
                f.write(f"Total configurations tested: {len(df['vector_db'].unique()) * len(df['llm_model'].unique())}\n")
            ...
            try:
                ...
                f.write(f"Error calculating vector DB statistics: {str(e)}")
            f.write("\n\n")
            ...
            try:
                ...
                f.write(f"Error calculating LLM statistics: {str(e)}")
            f.write("\n\n")
            ...
            try:
                ...
                f.write(f"Error calculating chunking statistics: {str(e)}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate RAG pipeline with all-MiniLM-L6-v2 model")
    parser.add_argument("--data-dir", type=str, default="data",
                      help="Directory containing test documents")
    args = parser.parse_args()
    
    evaluator = MiniLMPipelineEvaluator(args.data_dir)
