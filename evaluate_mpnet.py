"""Evaluation script for RAG pipeline with all-mpnet-base-v2 model."""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('mpnet_evaluation.log'),
...
VECTOR_DBS = {
    "chroma": lambda: ChromaDB(collection_name="mpnet_eval_collection"),
    "redis": lambda: RedisDB(collection_name="mpnet_eval_collection"),
    "qdrant": lambda: QdrantDB(collection_name="mpnet_eval_collection")
}
...
class MPNetPipelineEvaluator:
    """Evaluator for RAG pipeline with all-mpnet-base-v2 model."""
    ...
    def save_results(self):
        ...
        # Save to CSV
        csv_file = f"mpnet_evaluation_{timestamp}.csv"
        df = pd.DataFrame(self.results)
        df.to_csv(csv_file, index=False)
        
        # Save to JSON
        json_file = f"mpnet_evaluation_{timestamp}.json"
    ...
    def generate_summary(self, df: pd.DataFrame, timestamp: str):
        ...
        summary_file = f"mpnet_evaluation_summary_{timestamp}.txt"
        
        with open(summary_file, 'w') as f:
            f.write("=== RAG Pipeline Evaluation Summary (all-mpnet-base-v2) ===\n\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate RAG pipeline with all-mpnet-base-v2 model")
    parser.add_argument("--data-dir", type=str, default="data",
                      help="Directory containing test documents")
    args = parser.parse_args()
    
    evaluator = MPNetPipelineEvaluator(args.data_dir)
